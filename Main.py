...
class Main:
    ...
    def __init__(self):
        self.lastprintlen = 0
        self.Commands = {
        "create":{"args": {"path":{"help":"Specify path to the ct-config.json."}},"desc":"Create a template, by giving the path of the ct-config.json.", "help":"Create template.", "func":Main.create}}
        self.InitArgparse()

    def InitArgparse(self):
        parser = argparse.ArgumentParser(add_help=False, description=f"Clever Template - {Main.Version}") # epilog="For more information, read the documentation, or go to the official website."
